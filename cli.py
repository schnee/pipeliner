...
def main():
    ...
    parser.add_argument("-f", "--deals_file", help="Path to the deals CSV file", required=True)
    parser.add_argument("-t", "--target_revenue", type=float, help="Target revenue amount", required=True)
    parser.add_argument("-n", "--num_simulations", type=int, default=100000, help="Number of simulations to run (default: 100000)")
    parser.add_argument("-o", "--output_image", default="revenue_distribution.png", help="Path to save the output image (default: revenue_distribution.png)")
    ...
    output_image_path = os.path.abspath(args.output_image)
    fig.savefig(output_image_path)
    print(f"\nRevenue distribution plot saved to: {output_image_path}")
